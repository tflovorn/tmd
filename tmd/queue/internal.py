import os
import subprocess
import tmd.pwscf.config as pwscf.config

def enqueue(config):
    _write_queuefile(config)
...
def _write_queuefile_local(config):
    if config["command"] == "pw.x" or config["command"] == "pw2wannier90.x":
        file_prefix = pwscf.config.file_prefix(config)
